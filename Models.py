...
class GAN(object):
	...
	def discriminator(self,x, reuse=False): # input will always we a image
		with tf.variable_scope("Discriminator", reuse=reuse) as scope:
			D_conv1 = Conv_2D(x, output_chan=16, name="D_Conv1")
			D_conv2 = Conv_2D(D_conv1, output_chan=32, name="D_Conv2")
			D_conv3 = Conv_2D(D_conv2, output_chan=64, name="D_Conv3")
			D_conv4 = Conv_2D(D_conv3, output_chan=128, name="D_Conv4")
			D_conv4_r = tf.reshape( D_conv4, shape=[-1, int(np.prod(D_conv4.get_shape()[1:]))])
			D_linear5 = linear(D_conv4, output_dim=128, activation=None, name="D_linear5")
			return tf.sigmoid(D_linear5)

	def generator(self, x):
		assert x.get_shape()[1]==x.get_shape()[2]
		dim = x.get_shape()[1]
		with tf.variable_scope("Generator") as scope:
			G_linear1 = linear(x, output_dim=1024, name="G_hidden1")
			G_linear2 = linear(G_linear1, output_dim=dim*dim*128, name="G_hidden2")
			G_linear2_r = tf.reshape(G_linear2, shape=[-1,dim, dim, 128])
	...
	def Build_Model(self):
		with tf.name_scope("Inputs") as scope:
			...
			self.train_phase = tf.placeholder(tf.bool, "Trianing Phase")
			self.x_summ = tf.summary.image("Input Images", self.x)
			self.z_summ = tf.summary.histogram("Input Noise", self.z)

		with tf.name_scope("Model") as scope:
			self.gen_out = self.generator(z)
			self.dis_real = self.discriminator(x, reuse=False)
			self.dis_fake = self.discriminator(gen_out, reuse=True)
			self.gen_summ = tf.summary.image("Generator images", self.gen_out)

		with tf.name_scope("Loss") as scope:
			self.dis_fake_loss = tf.reduce_mean(-tf.log(dis_real))
			self.dis_real_loss = tf.reduce_mean(-tf.log(1- dis_fake))
			self.dis_loss = dis_real_loss + dis_fake_loss
			self.gen_loss = tf.reduce_mean(-tf.log(dis_fake))
