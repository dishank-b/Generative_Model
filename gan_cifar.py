...
class GAN(object):
	...
	def generator(self, z, batch_size):
		with tf.name_scope("generator") as sc:
			with tf.variable_scope("gen") as var_scope:
				...
				G_2 = Dense(G_h1, output_dim=4*4*128, name='G_hidden2') # [-1, 7*7*128]
				G_bn2 = BatchNormalization(G_2, name='G_hidden2')        
				G_h2 = tf.nn.relu(G_bn2)
				G_r2 = tf.reshape(G_h2, [-1, 4, 4, 128])
				...
				G_conv4 = Deconv2d(G_h3, output_dim=32, batch_size=batch_size, name='G_hidden4')
				G_bn4 = BatchNormalization(G_conv4, name='G_hidden4')        
				G_h4 = tf.nn.relu(G_bn4)
				G_conv5 = Deconv2d(G_h4, output_dim=3, batch_size=batch_size, name='G_hidden5')
				# print G_conv5.get_shape()
				G_r5 = tf.reshape(G_conv5, [-1, int(np.prod(G_conv5.get_shape()[1:]))])
				return G_conv5, tf.tanh(G_r5)
		...
		with tf.name_scope("discriminator") as sc:
			with tf.variable_scope("dis", reuse=reuse) as var_scope:
				if len(x.get_shape()) > 2:
					...
					D_reshaped = tf.reshape(x, [-1, 32, 32, 3])
					D_conv1 = Conv2d(D_reshaped, output_dim=32, name='D_hidden1')
				D_h1 = LeakyReLU(D_conv1) # [-1, 16, 16, 32]
				D_conv2 = Conv2d(D_h1, output_dim=64, name='D_hidden2')
				D_h2 = LeakyReLU(D_conv2) # [-1, 8, 8, 128]
				D_conv3 = Conv2d(D_h2, output_dim=128, name='D_hidden3')
				D_h3 = LeakyReLU(D_conv3) # [-1, 4, 4, 128]
	...
	def build_model(self, batch_size):
		with tf.name_scope("Inputs") as scope:
			...
			self.x = tf.placeholder(tf.float32, shape=[None, 3072]) # [batch_size, 32*32*3]
	...
	def train_model(self, images, learning_rate, epoch_size, batch_size, z_length):
		...
		for epoch in range(epoch_size):
			for itr in xrange(0, len(images)-batch_size, batch_size):
				...
				_, D_summary, D_loss, D_grads = self.sess.run([D_solver, self.D_summ, self.dis_loss, D_gradients], {self.z: batch_z, self.x:batch_images})
				self.writer.add_summary(D_summary, itr)

				# batch_z = np.random.uniform(-1,1, size=(batch_size, z_length))

				_, G_summary, G_loss, G_grads = self.sess.run([G_solver, self.G_summ, self.gen_loss, G_gradients], {self.z : batch_z, self.x:batch_images})
...
gan_model = GAN()
gan_model.build_model(batch_size=10)
gan_model.train_model(images = images,learning_rate=0.0001, epoch_size=200, batch_size=10, z_length=100)